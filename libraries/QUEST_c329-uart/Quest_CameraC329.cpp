/**
 * * ************************************************************
 * *******   QUEST S3 MICROLAB Version Uart  ********************
 * **************************************************************
 * 
 * Date = 20220329
 * This verision of QuestCameraC329 has been modified by hai to make it compatable
 * to the S3 Microlab system.... 
 * 
 * 
 *  
 * End of QUEST
 ...
 * Constructor
 */
Quest_CameraC329::Quest_CameraC329()
{
}
 ...
 * @return True if successful, false otherwise
 */
bool Quest_CameraC329::initialize(BaudRate baudRate, ColorType colorType, 
 ...
 */
bool Quest_CameraC329::setQuality(QualityLevel qualityLevel)
{
 ...
 * @return True if successful, false otherwise
 */
bool Quest_CameraC329::takeSnapshot(PictureType pictureType)
{
...
bool Quest_CameraC329::getPicture(PictureType pictureType, void (*callback)(uint32_t pictureSize, uint16_t packetSize, uint32_t packetStartPosition, byte* packet))
{
  uint32_t pictureSize = 0;

  setOutputCommand(CMD_GETPICTURE, pictureType, 0, 0, 0);
  sendCommand();
//	Serial.println("0");		//hai

  if (!waitForACK(RESPONSE_DELAY, CMD_GETPICTURE))
    {return false;
//	Serial.println("1");		//hai
}
  if (!(waitForResponse(RESPONSE_DELAY) && inputCommand[3] == CMD_DATA))
    {return false;
//	Serial.println("2");		//hai
}
...
//Serial.print("**************************** Retreived size = "); Serial.println(pictureSize);
 ...
 * @return True if successful, false otherwise.
 */
bool Quest_CameraC329::reset(ResetType resetType)
{
 ...
 * @return True if successful, false otherwise
 */
bool Quest_CameraC329::powerOff()
{
 ...
 */
void Quest_CameraC329::sendCommand()
{
  uint8_t i;
  ...
  for (i = 0; i < CMD_SIZE; i++)
  {
	...
	//Serial.print(outputCommand[i]);
	C329serial.write(outputCommand[i]);				//hai
  }
}

bool Quest_CameraC329::waitForACK(uint32_t timeout, uint8_t cmd)
{
 ...
 */
bool Quest_CameraC329::waitForResponse(uint32_t timeout)
{
 ...
 */
bool Quest_CameraC329::waitForResponse(uint32_t timeout, byte buffer[], uint16_t bufferLength)
{
  uint8_t byteCount = 0;
  ...
  {
    while (C329serial.available() > 0)
    {
      buffer[byteCount] = C329serial.read();
      byteCount++;
 ...
 */
void Quest_CameraC329::setOutputCommand(const byte command, byte param1, byte param2, byte param3, byte param4)
{
