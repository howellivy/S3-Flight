/**
 * ************************************************************
 * *******   QUEST S3 MICROLAB Version SPI ********************
 * ************************************************************
 * 
 * Date = 20230329
 * This verision of QuestCameraC329SPI has been modified by hai to make it compatable
 ...
 * Constructor
 */
QuestCameraC329SPI::QuestCameraC329SPI(int _spi_cs_pin):spi_cs_pin(_spi_cs_pin)
{
}
/*
...
*/
bool QuestCameraC329SPI::QuestCameraC329SPIinit()
{
 ...
 * SPIMAX_SYNC_ATTEMPTS. You must synchronize with the camera before you can
 * call other methods.
 *
 * @return True if successful, false otherwise.
 */
bool QuestCameraC329SPI::syncSPI()
{
  ...
  while (syncAttempts < SPIMAX_SYNC_ATTEMPTS)
  {
    sendCommand();
   // delay(1);  //*****************************

Serial.print(syncAttempts, HEX);Serial.print(" ");Serial.print(CMD_SYNC, HEX);    //test hai

    // Wait for ACK response
    success = waitForACK(SPIRESPONSE_DELAY, CMD_SYNC);

    // Make sure it is an ACK
    if (success)
    {
      // Now wait for SYNC from camera
      success = waitForResponse(SPIRESPONSE_DELAY);
 ...
 * @return True if successful, false otherwise
 */
bool QuestCameraC329SPI::initializeSPI(BaudRate baudRate, ColorType colorType, 
    PreviewResolution previewResolution, JPEGResolution jpegResolution)
{
  ...
  if (waitForACK(SPIRESPONSE_DELAY, CMD_INITIAL))
    return true;
 ...
 */
bool QuestCameraC329SPI::setQualitySPI(QualityLevel qualityLevel)
{
  setOutputCommand(CMD_QUALITY, qualityLevel, 0, 0, 0);
  sendCommand();

  if (waitForACK(SPIRESPONSE_DELAY, CMD_QUALITY))
    return true;
 ...
 * @return True if successful, false otherwise
 */
bool QuestCameraC329SPI::takeSnapshot(PictureType pictureType)
{
  setOutputCommand(CMD_SNAPSHOT, pictureType, 0, 0, 0);
  sendCommand();

  if (waitForACK(SPIRESPONSE_DELAY, CMD_SNAPSHOT))
    return true;

  return false;
}

bool QuestCameraC329SPI::getPictureSPI(PictureType pictureType, void (*callback)(uint32_t pictureSize, uint16_t packetSize, uint32_t packetStartPosition, byte* packet))
{
...
//  }
  ...
  while(get_pic_tries++ < 10) {       //-----------------------------hai
    ...
    while (!(waitForResponse(SPIRESPONSE_DELAY) && inputCommand[3] == CMD_DATA) && max_tries++ < total_tries) {
...
pictureSize=(x+y+z);
  ...
  uint8_t package[SPIDEFAULT_PACKAGE_SIZE];

  while (bytePosition < pictureSize)
  {
    if (!waitForResponse(SPIRESPONSE_DELAY, package, SPIDEFAULT_PACKAGE_SIZE))
      return false;
//	Serial.println(bytePosition);		//hai found boundry problem

    SPI.setClockDivider((F_CPU + 4000000L) / 8000000L); 

    callback(pictureSize, min(SPIDEFAULT_PACKAGE_SIZE, pictureSize - bytePosition), bytePosition, package);
    bytePosition += SPIDEFAULT_PACKAGE_SIZE;
  }
 ...
 * @return True if successful, false otherwise.
 */
bool QuestCameraC329SPI::resetSPI(ResetType resetType)
{
  setOutputCommand(CMD_RESET, resetType, 0, 0, 0xFF);
  sendCommand();

  if (waitForACK(SPIRESPONSE_DELAY, CMD_RESET))
    return true;
 ...
 * @return True if successful, false otherwise
 */
bool QuestCameraC329SPI::powerOffSPI()
{
  setOutputCommand(CMD_POWEROFF, 0, 0, 0, 0);
  sendCommand();

  if (waitForACK(SPIRESPONSE_DELAY, CMD_POWEROFF))
    return true;
 ...
 */
void QuestCameraC329SPI::sendCommand()
{
  uint8_t i;
...
bool QuestCameraC329SPI::waitForACK(uint32_t timeout, uint8_t cmd)
{
 ...
 */
bool QuestCameraC329SPI::waitForResponse(uint32_t timeout)
{
  return waitForResponse(timeout, inputCommand, SPICMD_SIZE);
}
 ...
 */
bool QuestCameraC329SPI::waitForResponse(uint32_t timeout, byte buffer[], uint16_t bufferLength)
{
  uint8_t byteCount = 0;
 ...
 */
void QuestCameraC329SPI::setOutputCommand(const uint8_t command, uint8_t param1, uint8_t param2, uint8_t param3, uint8_t param4)
{
